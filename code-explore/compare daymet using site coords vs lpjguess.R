#############################################################################
# Compare Daymet using site coords vs. LPJ-GUESS coords
# INCOMPLETE: coords basically the same. Need to look at model read-in
#############################################################################
rm(list=ls())
...
RC138j10 <- read.csv(paste(dpath, "ReynoldsC/138j10.wea.30min.2015.csv", sep="")) %>%
  mutate(Source="Station", Site="138h08")
...
df <- data.frame(map.p) %>%
  filter(x<=-116.66,x>=-116.77, y>=43.05,y<=43.18)
