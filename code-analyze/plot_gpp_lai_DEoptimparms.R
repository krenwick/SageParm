...
insfile <- "automate_tests/optim1_summergreen.ins" # name of ins file to use
...
tx  <- gsub(pattern = "Output_LHC_summergreen_optim1", replace = "Output_localruns", x = tx)
...
tx  <- gsub(pattern = "pstemp_maxval", replace = DE1$optim$bestmem[7], x = tx)
tx  <- gsub(pattern = "pstemp_hival", replace = DE1$optim$bestmem[8], x = tx)
tx  <- gsub(pattern = "aphenmaxval", replace = DE1$optim$bestmem[9], x = tx)
tx  <- gsub(pattern = "npatch 1", replace = "npatch 100", x = tx)
...
################################################################################
# Plot GPP:
ggplot(data=GPPall, aes(x=Date, y=GPP, color=Source, linetype=Source)) +
  geom_point() +
  geom_line() +
...
# Plot LAI:
ggplot(data=LAIall, aes(x=Date, y=LAI, color=Source, linetype=Source)) +
  geom_point() +
  geom_line() +
  scale_linetype_manual(name="Source",
                        breaks=c("Tower","Model","Model2"),
...
summary(lm(Tower~Model, data=R2gpp1)) # R2 = .70
summary(lm(Tower~Model2, data=R2gpp1)) # R2 = .72

R2lai <- LAIall %>% spread(Source,LAI)
summary(lm(Tower~Model, data=R2lai)) # R2 = .14
summary(lm(Tower~Model2, data=R2lai)) # R2 = .32
