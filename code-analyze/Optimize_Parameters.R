################################################################################
# Test implementing Levenberg-Marquardt NLS
# 
################################################################################
rm(list=ls())
library(tidyr)
library(minpack.lm)

# SET WORKING DIRECTORY:
setwd("~/Documents/SageParm")

# Read in monthly GPP data
df3 <- read.csv("data/RCflux_15_16.csv")
GPP <- df3 %>% filter(Variable=="GPP")

# Function to run LPJ-GUESS and return vector of residuals.
# First argument to function must be par
# Depends on ins and flux data already existing in memory

# First, read ins into memory:
ins  <- readLines("LPJfiles/Levenberg_M.ins") # only outputs GPP
LPJG <- function(par) {
  tx  <- gsub(pattern = "slaval", replace = par[1], x = ins)
  tx  <- gsub(pattern = "k_latosaval", replace = par[2], x = tx)
  tx  <- gsub(pattern = "gminval", replace = par[3], x = tx)
  tx  <- gsub(pattern = "ltor_maxval", replace = par[4], x = tx)
  tx  <- gsub(pattern = "greff_minval", replace = par[5], x = tx)
  tx  <- gsub(pattern = "rootdistval", replace = par[6], x = tx)
  tx  <- gsub(pattern = "rootdist2", replace = (1-par[6]), x = tx)
  tx  <- gsub(pattern = "turnover_sapval", replace = par[7], x = tx)
  tx  <- gsub(pattern = "pstemp_minval", replace = par[8], x = tx)
  tx  <- gsub(pattern = "pstemp_lowval", replace = par[9], x = tx)
  tx  <- gsub(pattern = "est_maxval", replace = par[10], x = tx)
  tx  <- gsub(pattern = "pstemp_maxval", replace = par[11], x = tx)
  tx  <- gsub(pattern = "pstemp_hival", replace = par[12], x = tx)
  tx  <- gsub(pattern = "k_chillbval", replace = par[13], x = tx)
  tx  <- gsub(pattern = "phengdd5rampval", replace = par[14], x = tx)
  tx  <- gsub(pattern = "leaflongval", replace = par[15], x = tx)
  writeLines(tx, con="LMopt/tempins.ins")
  
  # Run model using new ins file
  ptm <- proc.time()
  system("/Users/poulterlab1/version-control/LPJ-GUESS/ModelFiles/modules/./guess /Users/poulterlab1/Documents/SageParm/LMopt/tempins.ins")
  proc.time() - ptm # 111.265
  
  # Read in output from model
  mod <- fread("ModOut/Levenberg_M/mgpp.txt", header=T) %>% mutate(Year=Year+860) %>% 
    filter(Year>=2014) %>%
    gather(Month,Model, Jan:Dec) %>%
    mutate(Site=ifelse(Lon==-116.7486, "mbsec", "FIX")) %>%
    mutate(Site=ifelse(Lon==-116.7356, "losec", Site)) %>%
    mutate(Site=ifelse(Lon==-116.7132, "wbsec", Site)) %>%
    mutate(Site=ifelse(Lon==-116.7231, "h08ec", Site)) %>%
    mutate(Variable="GPP") %>%
    select(Year, Month,Variable,Site,Model)
  b <- merge(mod,GPP, by=c("Year","Month","Variable","Site"))
  return (b$Model - b$Tower)
  }


start <- c(14,3200,.5,1,.08,.6,.1,-4,10,.2,38,25,100,200,.5) # starting values
low <- c(6,1350,.35,.5,.056,.6,.05,-5.2,7,.05,26.6,17.5,100,100,.25) #upper bound for ech parameter (default is inf)
up <- c(21,5220,.65,1,.104,1,.1,-2.8,13,.2,49.4,32.5,600,300,1) #lower bound
t1 <- nls.lm(par=start,lower=low,upper=up,fn=LPJG, trace=T)
summary(t1)
print(t1)

# Finished
# > summary(t1)
# Error in chol.default(object$hessian) : 
#   the leading minor of order 4 is not positive definite
# > print(t1)
# Nonlinear regression via the Levenberg-Marquardt algorithm
# parameter estimates: 14, 3200, 0.5, 1, 0.08, 0.6, 0.1, -4, 10, 0.2, 38, 25, 100, 200, 0.5 
# residual sum-of-squares: 0.05135
# reason terminated: Relative error between `par' and the solution is at most `ptol'.
